...
Each activity takes about 3–5 minutes within group work, followed by a five-minute reuse discussion. Mentors support the conversation. Keep the final 10:45–11:45 sharing hour intact.
...
Everyone uses the same provenance structure. Source files are checked against the manifest; each record includes actual analytical settings and the notebook file hash. Student notes use roles and non-sensitive descriptions. Unknown decisions stay unresolved.

The record describes the notebook's in-memory results. For a future standalone export, preserve the matching record and add the exported artifact's filename/hash. Notebook hashes identify the saved file; save the notebook before creating a record you intend to retain.
